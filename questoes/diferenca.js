function startDiferenca(canvas, animFrameIdRef) {
    ...
    controls.innerHTML = `
        ...
        <div style="margin: 20px">
            <button onclick="aplicarOperacoes()">Aplicar Operações</button>
        </div>
    `;
    ...
    // Definição do filtro Box (passa-baixa)
    const filtroBox = [
        [1 / 9, 1 / 9, 1 / 9],
        [1 / 9, 1 / 9, 1 / 9],
        [1 / 9, 1 / 9, 1 / 9]
    ...
    window.aplicarOperacoes = function() {
        ...
        // Aplicar filtro Box na imagem original
        const imagemBox = aplicarConvolucao(imageData, filtroBox);

        // Calcular diferença absoluta entre original e filtrada
        const ctx1 = resultado1Canvas.getContext('2d', { willReadFrequently: true });
        const diferenca = calcularDiferencaAbsoluta(imageData, imagemBox);
        ctx1.putImageData(diferenca, 0, 0);
        desenharHistograma(diferenca, hist1Canvas, 'Histograma da Diferença Absoluta');

        // Aplicar filtro diferença diretamente
        const ctx2 = resultado2Canvas.getContext('2d', { willReadFrequently: true });
        const resultadoFiltroDiferenca = aplicarConvolucao(imageData, filtroDiferenca);
        ctx2.putImageData(resultadoFiltroDiferenca, 0, 0);
        desenharHistograma(resultadoFiltroDiferenca, hist2Canvas, 'Histograma do Filtro Diferença');
    };
    ...
    return function cleanup() {
        ...
        delete window.aplicarOperacoes;
    };
}